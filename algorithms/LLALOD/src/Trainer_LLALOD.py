...
from algorithms.LLALOD.src.dataloaders import dataloader_factory
from algorithms.LLALOD.src.models import model_factory
...
from algorithms.LLALOD.src.utils import *
...
class Trainer_LLALOD:
    ...
    def train(self):
        for cycle in range(self.args.active_cycle):
            ...
            sched_module   = torch.optim.lr_scheduler.MultiStepLR(optim_module, milestones=[240])
            ...
            with torch.no_grad():
                for (samples, boxes, labels, _)  in self.unlabeled_loader:
                    samples = samples.to(self.device)
                    predicted_locs, predicted_scores, features = self.model(samples)
                    pred_loss = self.loss_module(features)
                    pred_loss = pred_loss.view(pred_loss.size(0))
